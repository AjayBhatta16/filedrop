...
class SignupHandler():
    ...
    def handle(self, req):
        uname_query = self.user_repo.find({"username": req.username})

        if len(list(uname_query)) > 0:
            raise HttpException(400, "A user with this username already exists.")
        
        email_query = self.user_repo.find({"email": req.email})
